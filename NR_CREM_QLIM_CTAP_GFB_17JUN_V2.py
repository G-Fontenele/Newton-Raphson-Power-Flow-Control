...
iteracao_max = 20      # Número máximo de iterações
...
# DBAR_5: Usada para os caso 5.
DBAR_caso5 = [
    # Nº, Tipo, V(pu), Ang(°), Pg(MW), Qg(MVAr), Qmin,   Qmax, B.Rem.Ctrl, Pl(MW), Ql(MVAr), Shunt, Area,  Vf
    [1, 2, 1.060, 0.0,   232.4, -9.75, -999, 999, None,   0.0,   0.0,   0.0, 1, None],
    [2, 1, 1.045,  -5.0,   40.0,  50, -40.0, 50.0, None, 21.7,  12.7,  0.0, 1, None],
    [3, 1, 1.010,  -13.0,   0.0,  31.68,   0.0, 40.0, None, 94.2,  19.0,  0.0, 1, None],
    [4, 0, 1.004,  -10.0,   0.0,   0.0, -999, 999, None,  47.8,  -3.9,  0.0, 1, None],
    [5, 0, 1.010,  -8.7,    0.0,   0.0, -999, 999, None,   7.6,   1.6,  0.0, 1, None],
    [6, 1, 1.070,  -14,   0.0, 18.25,  -6, 24, 12, 11.2,   7.5,  0.0, 1, None],
    [7, 0, 1.000,  -13,   0.0,   0.0, -999, 999, None,   0.0,   0.0,  0.0, 1, None],
    [8, 1, 1.090,  -13,   0.0,  -6,  -6, 24, 7,  0.0,   0.0,  0.0, 1, None],
    [9, 0, 1.018,  -15,   0.0,   0.0, -999, 999, None,  29.5,  16.6, 19.0, 1, None],
    [10, 0, 1.023,  -15,  0.0,   0.0, -999, 999, None,   9.0,   5.8,  0.0, 1, None],
    [11, 0, 1.052,  -15,  0.0,   0.0, -999, 999, None,   3.5,   1.8,  0.0, 1, None],
    [12, 0, 1.070,  -15,  0.0,   0.0, -999, 999, None,   6.1,   1.6,  0.0, 1, None],
    [13, 0, 1.061,  -15,  0.0,   0.0, -999, 999, None,  13.5,   5.8,  0.0, 1, None],
    [14, 0, 1.018,  -16,  0.0,   0.0, -999, 999, None,  14.9,   5.0,  0.0, 1, None]
]
...
DLIN_caso5 = [
    ...
    [4, 7, 1, 0.0, 20.912, 0.0, 0.978, None, None, 0.0, None, None, None, None],
    [4, 9, 1, 0.0, 55.618, 0.0, 0.969, None, None, 0.0, None, None, None, None],
    [5, 6, 1, 0.0, 25.202, 0.0, 0.8965, 0.8, 1.2, 0.0, 5, None, None, None],
    [6, 11, 1, 9.498, 19.890, 0.0, 1.0, None, None, 0.0, None, None, None, None],
    [6, 12, 1, 12.291, 25.581, 0.0, 1.0, None, None, 0.0, None, None, None, None],
    [6, 13, 1, 6.615, 13.027, 0.0, 1.0, None, None, 0.0, None, None, None, None],
    [7, 8, 1, 0.0, 17.615, 0.0, 1.0, None, None, 0.0, None, None, None, None],
...
def controle_tensao_reversivel(rem_info, QG, QM, QN, V, Vb_controlada, Qesp, QD, TIPO, Pbase, Ch, NBAR, iteracao_atual):
    print(f"\n--- Iteração {iteracao_atual}: Verificando Controles Reversíveis ---")
    for c in range(rem_info['n_controle']):
        ...
        print(f"  > Gerador {idx_controle+1} (Ctrl Barra {idx_controlada+1}): QG={QG[idx_controle]:.2f} [Min:{QN[idx_controle]:.2f}, Max:{QM[idx_controle]:.2f}], Vctrl={Vctrl:.4f} (Vref={Vref:.4f})")
...
def newton_raphson_integrado(DBAR, DLIN, Pbase, tolerancia, tol_Vcontrolada, iteracao_max, printar_relatorio=True):
    ...
    while not convergiu and i < iteracao_max:
        ...
        all_controls = Controle_CTAP and Controle_REM and Controle_PV
        if Controle_CTAP and not tap_info['ativo']:
            ...
            if not Controle_REM:
                # Caso não esteja com outros controles ativos, já ativa o Tap de início
                tap_info['ativo'] = True
                print(f"→ CTAP ativado na iteração {i} (por conta de ser o único controle)")
            else:
                if max_dq < TPST:
                    tap_info['ativo'] = True
                    print(f"→ CTAP ativado na iteração {i} (max |ΔQ| = {max_dq:.8f} < {TPST:.8f})")

        if printar_relatorio:
            print(f"Mismatch Potência: {erro_pot:.7f} | Mismatch Controle: {erro_ctrl:.7f}")
            print(f"Tol de Potência: {tolerancia:.7f} | Mismatch Controle: {tol_Vcontrolada:.7f}")

        # Foi necessário adicionar uma verificação de convergência para condições em que a tolerância é muito pequena e ambas as tolerâncias se aproximam muito da solução para evitar divergência
        convergencia_para_tol_pequena = (all_controls and np.abs(erro_pot - tolerancia) < 1e-6) 
        print(f" >> Resultado (convergencia_para_tol_pequena): {convergencia_para_tol_pequena} {np.abs(erro_ctrl - tolerancia)}")

        if erro_ctrl < tol_Vcontrolada and (erro_pot < tolerancia or convergencia_para_tol_pequena):
            convergiu = True
            break
        ...
        controle_tensao_reversivel(rem_info, QG, QM, QN, V, rem_info['v_esp'], Qesp, QD, TIPO, Pbase, Ch, NBAR, i)        
...
if __name__ == '__main__':
    ...
    match escolha:
        ...
        case 5:
            ...
            DBAR, DLIN = DBAR_caso5, DLIN_caso5
            Controle_PV = True
            Controle_REM = True
            Controle_CTAP = True
        case _: # Default case

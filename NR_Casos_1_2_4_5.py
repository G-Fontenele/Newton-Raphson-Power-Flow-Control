...
DLIN = [
    ...
    [4, 9, 1, 0.0, 55.618, 0.0, 0.969, 0.6, 1.4, 0.0, None, None, None, None],
...
def montar_matriz_jacobiana_aumentada(NBAR, V, TETA, Pcalc, Qcalc, G, B, TIPO, R, X, controles, Ch):
    ...
    # ### LÓGICA DO CTAP CORRIGIDA ###
    # if Controle_CTAP and N_CTAP > 0:
    #     # CASO 1: O controle está ATIVO. Preenche a Jacobiana com as derivadas.
    #     if tap_info['ativo']:
    ...
    #             gkm, bkm = G[k_idx, m_idx], B[k_idx, m_idx]
    #             Vk, Vm, teta_k, teta_m = V[k_idx], V[m_idx], TETA[k_idx], TETA[m_idx]
    ...
    #     # CASO 2: O controle está INATIVO. Trava a variável delta_TAP em zero para evitar singularidade.
    #     else:
    #         for i in range(N_CTAP):
    #             idx_var_tap = 2 * NBAR + N_REM + i
    #             Jac[idx_var_tap, idx_var_tap] = 1e10 # Equivale a delta_TAP = 0
# Bloco de código CORRETO e CONSISTENTE para a convenção de tap 'a'
    if Controle_CTAP and N_CTAP > 0:
        if tap_info['ativo']:
            for i in range(N_CTAP):
                ramal_idx = tap_info['indices_ramos'][i]
                ...
                r_km, x_km = R[ramal_idx], X[ramal_idx]
                tap = tap_info['taps'][ramal_idx]
                Vk, Vm = V[k_idx], V[m_idx]
                teta_km = TETA[k_idx] - TETA[m_idx]

                z_sq = r_km**2 + x_km**2
                g_km = r_km / z_sq
                b_km = -x_km / z_sq

                # Derivadas em relação a 'a'
                # Na barra K (lado do tap)
                # dPk_da = (2 * Vk**2 / tap**3) * g_km - (Vk * Vm / tap**2) * (g_km * np.cos(teta_km) + b_km * np.sin(teta_km))
                # dQk_da = (2 * Vk**2 / tap**3) * b_km + (Vk * Vm / tap**2) * (g_km * np.sin(teta_km) - b_km * np.cos(teta_km))
                # # Na barra M
                # dPm_da = (Vk * Vm / tap**2) * (g_km * np.cos(teta_km) + b_km * np.sin(teta_km))
                # dQm_da = (Vk * Vm / tap**2) * (g_km * np.sin(teta_km) - b_km * np.cos(teta_km))
                dPk_da = (-2 * Vk**2 / tap**3) * g_km + (Vk * Vm / tap**2) * (g_km * np.cos(teta_km) + b_km * np.sin(teta_km))
                dQk_da = (2 * Vk**2 / tap**3) * b_km + (Vk * Vm / tap**2) * (g_km * np.sin(teta_km) - b_km * np.cos(teta_km))
                dPm_da = (Vk * Vm / tap**2) * (g_km * np.cos(teta_km) + b_km * np.sin(teta_km))
                dQm_da = (-Vk * Vm / tap**2) * (g_km * np.sin(teta_km) - b_km * np.cos(teta_km))

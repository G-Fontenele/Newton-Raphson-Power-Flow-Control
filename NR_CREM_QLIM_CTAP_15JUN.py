...
Controle_PV = True      # Controle de limites de potência reativa (PV -> PQ)
Controle_REM = True     # Controle remoto de tensão por gerador (CREM)
...
tol = 0.00001 / Pbase      # Tolerância de convergência para potências (em pu)
...
# -------------------------------------------------------------------------------------
DBAR = [
    [1, 2, 1.060,   0.0,  232.8, -9.75, -999, 999, None,   0.0,   0.0,  0.0, 1, None],
    [2, 1, 1.045,  -5.0,   40.0,  50, -40.0, 50.0, None,  21.7,  12.7,  0.0, 1, None],
    [3, 1, 1.010, -13.0,    0.0,  31.68,   0.0, 40.0, None,  94.2,  19.0,  0.0, 1, None],
    [4, 0, 1.008, -10.0,    0.0,    0.0, -999, 999, None,  47.8,  -3.9,  0.0, 1, None],
    [5, 0, 1.010,  -8.8,    0.0,    0.0, -999, 999, None,   7.6,   1.6,  0.0, 1, None],
    [6, 1, 1.070, -14.0,    0.0,  18.25,  -6.0, 24.0, 12,  11.2,   7.5,  0.0, 1, None],
    [7, 0, 1.000, -13.0,    0.0,    0.0, -999, 999, None,   0.0,   0.0,  0.0, 1, None],
    [8, 1, 1.090, -13.0,    0.0,  -6.0,  -6.0, 24.0, 7,   0.0,   0.0,  0.0, 1, None],
    [9, 0, 1.038, -15.0,    0.0,    0.0, -999, 999, None,  29.5,  16.6, 19.0, 1, None],
    [10,0, 1.039, -15.0,    0.0,    0.0, -999, 999, None,   9.0,   5.8,  0.0, 1, None],
    [11,0, 1.059, -15.0,    0.0,    0.0, -999, 999, None,   3.5,   1.8,  0.0, 1, None],
    [12,0, 1.070, -15.0,    0.0,    0.0, -999, 999, None,   6.1,   1.6,  0.0, 1, None],
    [13,0, 1.063, -15.0,    0.0,    0.0, -999, 999, None,  13.5,   5.8,  0.0, 1, None],
    [14,0, 1.031, -16.0,    0.0,    0.0, -999, 999, None,  14.9,   5.0,  0.0, 1, None]
]
...
DLIN = [
    ...
    [4,  9, 1,    0.0,  55.618,   0.00, 0.969,  0.8,   1.2,   0.0, None, None, None, None],
    [5,  6, 1,    0.0,  25.202,   0.00, 0.850,  0.8,   1.2,   0.0, 5, None, None, None],
...
def montar_matriz_jacobiana_aumentada(NBAR, V, TETA, Pcalc, Qcalc, G, B, TIPO, controles, Ch):
    ...
    if Controle_CTAP and N_CTAP > 0:
        for i in range(N_CTAP):
            ...
            teta_k = TETA[k_idx]
            teta_m = TETA[m_idx]
            tap = tap_info['taps'][ramal_idx] if tap_info['taps'][ramal_idx] != 0 else 1.0

            # Índice da nova variável de controle TAP
            idx_var_tap = 2 * NBAR + N_REM + i

            # Derivadas conforme nova formulação
...
def newton_raphson_integrado(DBAR, DLIN, Pbase, tolerancia, tol_Vcontrolada, iteracao_max, printar_relatorio=True):
    ...
    while not convergiu and i < iteracao_max:
        ...
        if Controle_CTAP and tap_info['n_controle'] > 0 and printar_relatorio:
